...
static const uint32_t SCAN_WINDOW_MS = 5000;
static const uint32_t SCAN_RETRY_MS = 6000;
...
bool connectToServer() {
    ...
    if (s_bleClient == nullptr) {
      s_bleClient = BLEDevice::createClient();
      s_bleClient->setClientCallbacks(new MyClientCallback());
    }

    if (!s_bleClient->connect(myDevice)) {
      doScan = true;
      return false;
    }
    Serial.println("Connected to server");

    BLERemoteService* pRemoteService = s_bleClient->getService(serviceUUID);
...
static void start_scan_if_needed(void) {
  ...
  if ((now - s_last_scan_ms) < SCAN_RETRY_MS) {
    return;
  }

  Serial.println("Scanning for robot...");
  BLEDevice::getScan()->start(SCAN_WINDOW_MS / 1000, false);
  s_last_scan_ms = now;
}
