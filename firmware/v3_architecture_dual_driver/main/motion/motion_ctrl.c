...
// =====================================================================
//  PIN DEFINITIONS (identical to V1 — no hardware changes)
// =====================================================================
...
static mcpwm_cmpr_handle_t setup_servo(int pin, int group_id) {
    ...
    mcpwm_operator_config_t opcfg = { .group_id = group_id };
    ESP_ERROR_CHECK(mcpwm_new_operator(&opcfg, &oper));
...
static void init_motors(void) {
    uint8_t pins[] = {LEFT_RPWM, LEFT_LPWM, RIGHT_RPWM, RIGHT_LPWM};
...
static void set_motor_speeds(int lr, int ll, int rr, int rl) {
    ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_0, lr);
    ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_1, ll);
    ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_2, rr);
    ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_3, rl);
    for (int i = 0; i < 4; i++) ledc_update_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_0 + i);
}
...
void task_motion(void *arg) {
    ESP_LOGI(TAG, "Initializing Curie V3.0 Stable Motion System (2-Driver Mode)...");

    // Init servos
    cmp_ls = setup_servo(LEFT_SHOULDER_PIN, 0);
    cmp_le = setup_servo(LEFT_ELBOW_PIN, 0);
    cmp_rs = setup_servo(RIGHT_SHOULDER_PIN, 1);
    ...
    while (1) {
        ...
        if (dir != last_dir || actual_speed > 0 || last_dir != DIR_STOP) {
            ...
            switch (dir) {
                case DIR_FWD:       set_motor_speeds(s,0, s,0); break;
                case DIR_BWD:       set_motor_speeds(0,s, 0,s); break;
                case DIR_LEFT:      set_motor_speeds(0,s, s,0); break;
                case DIR_RIGHT:     set_motor_speeds(s,0, 0,s); break;
                case DIR_FWD_LEFT:  set_motor_speeds(h,0, s,0); break;
                case DIR_FWD_RIGHT: set_motor_speeds(s,0, h,0); break;
                case DIR_BWD_LEFT:  set_motor_speeds(0,h, 0,s); break;
                case DIR_BWD_RIGHT: set_motor_speeds(0,s, 0,h); break;
                case DIR_STOP:
                default:            set_motor_speeds(0,0, 0,0); break;
            }
            last_dir = dir;
        }
