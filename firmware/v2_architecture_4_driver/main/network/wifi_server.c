...
static esp_err_t ws_handler(httpd_req_t *req) {
    ...
    } else if (strcmp(cstr, "profile") == 0) {
        cJSON *prof = cJSON_GetObjectItem(json, "profile");
        if (cJSON_IsString(prof)) {
            if (strcmp(prof->valuestring, "smooth") == 0) payload.profile = PROFILE_SMOOTH;
            else if (strcmp(prof->valuestring, "aggressive") == 0) payload.profile = PROFILE_AGGRESSIVE;
...
static void start_ap_mode(void) {
    ...
    strcpy(s_net_info.ssid, "Infrared Curie Setup");
    s_net_info.rssi = 0;
    ...
    wifi_config_t wifi_config_ap = {
        .ap = {
            ...
            .ssid_len = strlen("Infrared Curie Setup"),
            .channel = 1,
            .password = "",
            .max_connection = 4,
